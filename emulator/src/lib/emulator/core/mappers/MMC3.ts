...
const MMC3_MIRRORS = <{ [key: number]: number }>{
...
class MMC3 extends Mapper {
  ...
  public bankRegister: number;
  public irqCounter: number;
  public irqLatch: number;
  public irqEnable: boolean;
  ...
  control(address: number, value: number) {
    ...
    } else if (address < 0xbfff && isEven) {
      this.mirrorType = MMC3_MIRRORS[value & 1];
    } else if (address < 0xc000 && !isEven) {
      // Write Protect, not implemented
    } else if (address < 0xdfff && isEven) {
      this.irqLatch = value;
    } else if (address < 0xe000 && !isEven) {
      this.irqCounter = 0;
    } else if (address < 0xffff && isEven) {
      this.irqEnable = false;
    } else if (address < 0x10000 && !isEven) {
      this.irqEnable = true;
    } else {
      throw new Error("Unknown control: " + address);
    }
  }
}
