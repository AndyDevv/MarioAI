...
export const modes = {
  ...
  [MODES.ABSOLUTE_X]: cpu => {
    const addr = (cpu.read16(cpu.pc + 1) + cpu.x) & 0xffff;
  ...
  [MODES.ABSOLUTE_Y]: cpu => {
    const addr = (cpu.read16(cpu.pc + 1) + cpu.y) & 0xffff;
  ...
  [MODES.INDIRECT_INDEXED_Y]: cpu => {
    const addr = (cpu.read16indirect(cpu.read8(cpu.pc + 1)) + cpu.y) & 0xffff;
